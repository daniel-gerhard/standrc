waic <- function(object){ 
  ...
  log_lik <- extract(object$stan, "log_lik")$log_lik 
  ...
  return(list(waic=total["waic"], elpd_waic=total["elpd_waic"], p_waic=total["p_waic"], elpd_loo=total["elpd_loo"], p_loo=total["p_loo"], pointwise=pointwise, total=total, se=se)) 
}