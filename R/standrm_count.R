standrm_count <- function(formula, data, fct, curveid=NULL, random=NULL, priors=standrc_priors(), ...){
  ...
  if (mean(y[x == min(x)]) < mean(y[x == max(x)])){
    ...
    if (is.null(priors$pd)) pd <- rep(log(max(y)), jv[3]) else pd <- priors$pd
  } else {
    if (is.null(priors$pd)) pd <- rep(-10, jv[3]) else pd <- priors$pd
    if (is.null(priors$pc)) pc <- rep(log(max(y)), jv[2]) else pc <- priors$pc
  }  
  ...
  if (is.null(random)){
    trap <- c("slope", "exp(lasy)", "exp(uasy)", "ed", "assym")[!isfix]
    traj <- c("slope[idc[i]]", "exp(lasy[idc[i]])", "exp(uasy[idc[i]])", "ed[idc[i]]", "assym[idc[i]]")[!isfix]
  } else {
  ...
  if (is.null(random)){
    ...
    trar <- c(paste("(", tra[1], " + rslope[idr[i]] )"), 
              paste("exp(", tra[2], " + rlasy[idr[i]] )"), 
              paste("exp(", tra[3], " + ruasy[idr[i]] )"), 
  ...
  pof[isfix] <- po[isfix]
  ppc <- paste(pof[1], " <- ", pp[1], if (pnlr[1]) " + rnslope", ";", pof[2], " <- ", if (!isfix[2]) "exp(", pp[2], if (pnlr[2]) " + rnlasy", if (!isfix[2]) ")", ";", pof[3], " <- ", if (!isfix[3]) "exp(", pp[3], if (pnlr[3]) " + rnuasy", if (!isfix[3]) ")", ";", pof[4], " <- ", pp[4], if (pnlr[4]) " + rned", ";", pof[5], " <- ", pp[5], if (pnlr[5]) " + rnassym", ";")  
  ...
  stancode <- paste("data {", 
                    ...
                    "log_lik[i] <- poisson_log(y[i], mu[i]);",
                    "}",
                    ppc,                                     
                    "}",
                    sep="")
